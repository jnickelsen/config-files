...
;;   (cl-pushnew theme my/favorite-themes)
...
;;                 (or (cl-position index-or-name names :test #'string=) 0))
...
;;               (set-face-attribute face 'unspecified :foreground (nth i color-theme))))
...
;;           (let* ((face (car pair))
;;                  (color-idx (cdr pair))
;;                  (color (nth (1- color-idx) colors)))
;;             (when color
;;               (set-face-attribute face 'unspecified
;;                                   :foreground (if (member face '(markdown-code-face))
;;                                                   nil
;;                                                 color)
...
;;   "Apply Org heading and Markdown palettes safely."
;;   (my/apply-org-heading-palette palette-name)
;;   (if (featurep 'markdown-mode)
;;       (my/apply-markdown-palette-by-name palette-name)
;;     (add-hook 'markdown-mode-hook
;;               (lambda () (my/apply-markdown-palette-by-name palette-name)))))
...
;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; ;; Daily themes + palettes (global, no frames)
...
;;     ("Wednesday" . (:theme doom-ayu-light       :palette "Lavender & Purples"))
...
;;   "Load theme and org/markdown palettes based on day and time."
;;   (interactive)
;;   (let* ((day (format-time-string "%A"))
;;          (alist (if (evening-hours-p)
;;                     jess/night-theme-palette-alist
;;                   jess/day-theme-palette-alist))
;;          (entry (or (assoc day alist) (assoc '_' alist)))
...
;; (when (fboundp 'timer-list)
;;   (mapc (lambda (t)
;;           (when (and (timerp t)
;;                      (symbolp (timer--function t))
;;                      (string-match-p "daily-theme" (symbol-name (timer--function t))))
;;             (cancel-timer t)))
;;         (timer-list)))

;; ;; Set up global timers safely
;; (defvar jess/daily-theme-global-timers nil
;;   "Holds the global timers for daily theme updates.")

;; (setq jess/daily-theme-global-timers
;;       (list
;;        ;; Hourly timer
...
;; ;; Apply immediately on load
;; (jess/apply-daily-theme-and-palette)
