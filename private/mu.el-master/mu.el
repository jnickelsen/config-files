...
(defun mu-comint-output-filter-advice (orig-fun &rest args)
  "MU's wrapper advice function for `comint-output-filter'.

Wraps the underlying function to inhibit recording server-message
insertion in the undo data."
  (let ((buffer-undo-list t))
    (apply orig-fun args)))

(defun mu-preoutput-filter (text)
  "Comint preoutput filter function for mu connections."
...
\\{mu-connection-mode-map}"
  ...
  (add-function :around (symbol-function #'comint-output-filter) #'mu-comint-output-filter-advice)
  (add-hook 'comint-preoutput-filter-functions #'mu-preoutput-filter t t)
  ;; User stuff.
